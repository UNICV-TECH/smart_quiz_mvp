...
class AppRoutes {
  ...
  static Map<String, WidgetBuilder> getRoutes() {
    return {
      /// A rota '/' é a entrada principal. No Web, ela carrega a SplashScreen
      /// que por sua vez decide se vai para Welcome ou Home.
      ...
      // Tela onde o usuário digita o e-mail para recuperar senha
      resetPassword: (context) => ChangeNotifierProvider(
            create: (context) => ForgotPasswordViewModel(
              authService: context.read<AuthService>(),
            ),
            child: const ResetPasswordScreen1(),
          ),

      // Tela onde o usuário digita a NOVA SENHA após clicar no link do e-mail
      resetPassword2: (context) => ChangeNotifierProvider(
            create: (context) => ResetPasswordViewModel(
              authService: context.read<AuthService>(),
            ),
            child: const ResetPasswordScreen2(),
          ),
      ...
      quizConfig: (context) => ProtectedRoute(
            builder: (innerContext) {
              ...
              if (course == null) {
                return const Scaffold(
                  body: Center(child: Text('Missing course data')),
                );
              }
      ...
      examResult: (context) => ProtectedRoute(
            builder: (innerContext) {
              ...
              if (args == null) {
                return const Scaffold(
                  body: Center(child: Text('Missing result data')),
                );
              }
