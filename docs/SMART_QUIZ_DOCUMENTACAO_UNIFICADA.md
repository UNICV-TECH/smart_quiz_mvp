# SMART QUIZ - Documentacao Tecnica Completa e Unificada

> Projeto de Extensao - Centro Universitario UniCV
> Versao: 1.0.0 | Ultima atualizacao: Marco 2026
...
A aplicacao e construida com **Flutter** para frontend multiplataforma e **Supabase** (PostgreSQL + Auth) como backend, suportando tres papeis de usuario: **student**, **teacher** e **admin**, cada um com modulos e interfaces dedicados.
...
- Permita que administradores gerenciem a plataforma de forma completa

### 1.3 Publico-Alvo

Uso exclusivo por **alunos, professores e administradores da UniCV**. O sistema possui tres papeis:
...
| **admin** | Administrador do sistema | Tudo de teacher + gestao completa de usuarios e plataforma |
...
#### RF11 - Modulo do Administrador
| ID | Descricao | Prioridade |
|----|-----------|-----------|
| RF11.1 | O admin deve poder listar todos os usuarios com estatisticas | Alta |
| RF11.2 | O admin deve poder alterar roles de usuarios (impede auto-rebaixamento) | Alta |
| RF11.3 | O admin deve poder ativar/desativar usuarios (soft delete, impede auto-desativacao) | Alta |
| RF11.4 | O admin deve ter acesso a analytics da plataforma | Media |
...
O projeto segue o padrao **MVVM (Model-View-ViewModel)** com **Provider + ChangeNotifier** para gerenciamento de estado.
...
│   └── ui/
│       ├── theme/
│       │   ├── app_color.dart           # Paleta de cores (verde institucional)
│       │   └── string_text.dart         # Estilos de texto
│       ├── components/                  # ~25 componentes reutilizaveis (prefixo default_)
│       └── preview.dart
├── supabase/migrations/                # 18+ arquivos SQL de migracao
...
└── docs/
    ...
    └── SMART_QUIZ_DOCUMENTACAO_UNIFICADA.md  # Este documento
```
...
Todos os modelos ficam em `lib/models/` e seguem o padrao de factories `fromJson`/`toJson` com `copyWith` para imutabilidade.

### 4.1 Tabela Resumo de Modelos
...
| `GamificationSeason` | `gamification_season.dart` | id, name (ex: "2026.1"), startsAt, endsAt, isActive |
...
| `TeacherQuestion` | `teacher_question.dart` | Visao de questoes do professor com dados agregados |
| `TeacherStats` | `teacher_stats.dart` | TeacherQuestionStats, TeacherExamStats, TeacherDashboardStats |

### 4.2 Detalhamento dos Modelos Principais
...
### 5.1 Repositorios de Autenticacao (`lib/repositories/auth/`)
...
**Politica "Best Score Only":** Apenas melhorias sobre a melhor pontuacao anterior sao registradas (delta salvo).
...
### 7.2 ViewModels do Professor (`lib/viewmodels/teacher/`)
...
| 1 | Ranking | RankingScreen (3 sub-abas: Global, Por Curso, Por Prova) | emoji_events |
...
O widget `ProtectedRoute` (`lib/widgets/protected_route.dart`) verifica autenticacao via `SessionManager`:
...
- `lib/ui/theme/app_color.dart` — Paleta de cores (verde institucional)
- `lib/ui/theme/string_text.dart` — Estilos de texto
- **Design System:** Material Design 3 com fonte Poppins (Google Fonts)
...
| idquestion | uuid | Sim | | FK → question(id) (sem underscore - legado) |
...
#### `admin_list_users(...)`
Lista todos os usuarios com estatisticas (apenas admin).

#### `admin_update_user_role(...)`
Atualiza roles de usuarios (impede auto-rebaixamento).

#### `admin_toggle_user_active(...)`
Soft delete de usuarios (impede auto-desativacao).
...
**Inconsistencias de nomenclatura (legado vs. moderno, preservadas para compatibilidade):**
...
As migracoes estao em `supabase/migrations/` e devem ser aplicadas em ordem numerica via Supabase CLI (`supabase db push`) ou manualmente no SQL Editor do Dashboard.

**Migracoes-chave:**

| Migracao | Descricao |
|----------|-----------|
| `20250204000001` | Modulo do professor (roles, categorias, templates) |
| `20250225000001` | Funcao de atualizacao de questao do professor |
| `20250225000002` | Tabela subject |
| `20260224000001` | Tabelas de gamificacao e views de ranking |
| `20260303000001` | Modulo admin (analytics, gestao de usuarios) |
...
### 14.3 Configuracao de Ambiente

- **Web:** Usa `--dart-define` para SUPABASE_URL e SUPABASE_ANON_KEY (definido no CI/deploy)
- **Nativo:** Carrega de arquivo `assets/dotenv.env`
- **Configuracao acessada via:** `lib/constants/supabase_options.dart`

### 14.4 Configuracao de Secrets (CI/CD)
...
### 18.1 Setup e Execucao

```bash
# Instalar dependencias
flutter pub get

# Executar em device conectado/emulador
flutter run

# Executar no Chrome (web)
flutter run -d chrome

# Executar com variaveis de ambiente locais
...
```bash
# Build web (producao usa --dart-define para secrets)
flutter build web --release
...
```bash
# Aplicar migracoes via Supabase CLI
supabase db push

# Ou manualmente no SQL Editor do Dashboard Supabase
# Executar cada arquivo em supabase/migrations/ na ordem do nome
```
...
> **Fontes:** CLAUDE.md + docs/DOCUMENTACAO_COMPLETA.md (unificados)
